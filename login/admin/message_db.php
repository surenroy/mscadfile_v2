...
switch ($action) {
    ...
    case 'sendMessage':
        ...
        $sql="INSERT INTO `message` (`from`,`to`,`create_on`,`subject`) VALUES ('$user_id','$touser',NOW(),'$subject')";
    ...
    case 'load_inbox':
        ...
        $sql="SELECT message.id, message.open, message.subject,DATE_FORMAT(`create_on`, '%d-%m-%Y %H:%i') AS date_time,
           message.open,message.reply,users.name FROM message message
        ...
        foreach ($my_arr as $val) {
            ...
            if($reply==''){
                $reply_t='<button class="btn btn-sm btn-primary reply'.$val['id'].' mx-1 p-1 px-2" onclick="add_reply('.$val['id'].')"><i class="fa-solid fa-reply"></i></button>';
            }else{
                $reply_t='';
            }
    ...
    case 'reply_message':
        ...
        $sql="SELECT `from`,`subject` FROM `message` WHERE `id`='$id'";
        $query = $pdoconn->prepare($sql);
        $query->execute();
        $my_arr = $query->fetchAll(PDO::FETCH_ASSOC);

        $touser=$my_arr[0]['from'];
        $subject='Reply: '.$my_arr[0]['subject'];
        ...
        if($result==0){
            ...
            $sql="UPDATE `message` SET `reply`='$lastInsertId' WHERE `id`='$id'";
            $query = $pdoconn->prepare($sql);
            $query->execute();
