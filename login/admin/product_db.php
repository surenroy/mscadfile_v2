...
switch ($action) {
           ...
           $total_files=$val['total_files'];
            ...
            if($pending==1 && $drive_pending==0){
                ...
                $html.='<tr>
                    ...
                    <td>
                        <a class="btn btn-sm btn-dark mx-1 p-1 px-2" href="'.$site_url.'product/?name='.$slug.'" target="_blank"><i class="fa-solid fa-eye"></i></a>
    ...
    case 'change_status':
        ...
        if($active==1){
            $sql="UPDATE `products` SET `active`='0',`admin_stop`='1' WHERE `id`='$id' AND `user_id`='$user_id'";
        }else if($active==0){
            $sql="UPDATE `products` SET `active`='1',`admin_stop`='0' WHERE `id`='$id' AND `user_id`='$user_id'";
        }
        $query = $pdoconn->prepare($sql);
        $query->execute();
